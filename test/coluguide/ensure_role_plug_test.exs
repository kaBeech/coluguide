defmodule ColuguideWeb.EnsureRolePlugTest do
  use ColuguideWeb.ConnCase

  alias ColuguideWeb.EnsureRolePlug
  ...
  setup do
    conn =
      ...
      |> Pow.Plug.put_config(otp_app: :coluguide)
      |> fetch_flash()
  ...
  test "call/2 with non-admin user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@user, otp_app: :coluguide)
  ...
  test "call/2 with admin user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@admin, otp_app: :coluguide)
      |> EnsureRolePlug.call(opts)
  ...
  test "call/2 with editor user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@editor, otp_app: :coluguide)
      |> EnsureRolePlug.call(opts)
