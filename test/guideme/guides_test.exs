defmodule Guideme.GuidesTest do
  ...
  describe "guides" do
    ...
    @invalid_attrs %{title: nil, category: nil, chapter: nil, short_title: nil, icon: nil}
    ...
    test "create_guide/1 with valid data creates a guide" do
      valid_attrs = %{title: "some title", category: "some category", chapter: "some chapter", short_title: "some short_title", icon: "some icon"}

      assert {:ok, %Guide{} = guide} = Guides.create_guide(valid_attrs)
      assert guide.title == "some title"
      assert guide.category == "some category"
    ...
    test "update_guide/2 with valid data updates the guide" do
      guide = guide_fixture()
      update_attrs = %{title: "some updated title", category: "some updated category", chapter: "some updated chapter", short_title: "some updated short_title", icon: "some updated icon"}

      assert {:ok, %Guide{} = guide} = Guides.update_guide(guide, update_attrs)
      assert guide.title == "some updated title"
      assert guide.category == "some updated category"
